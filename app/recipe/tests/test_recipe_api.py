...
class PrivateRecpeApiTests(TestCase):
    ...
    def test_delete_recipe(self):
        """Tets deleting a recipe successful."""
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            "time_minutes": 30,
            "price": Decimal("5.50"),
            "ingredients": [{"name": "Chicken"}, {"name": "Tikka"}],
        }
        res = self.client.post(RECIPES_URL, payload, format="json")
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        ingredient_tikka = Ingredient.objects.create(user=self.user, name="Tikka")
        payload = {"ingredients": [{"name": "Tikka"}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format="json")

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingredient_tikka, recipe.ingredients.all())
